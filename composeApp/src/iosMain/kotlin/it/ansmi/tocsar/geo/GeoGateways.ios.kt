...
private fun GeoFix.isFresh(now: Long = nowMs()): Boolean {
    if (timestampMs <= 0L) return true
    return now - timestampMs <= GpsPublishPolicy.MAX_FIX_AGE_MS
}
...
private class IosLocationGateway : LocationGateway {
    ...
    override suspend fun currentFix(): GeoFix? = withContext(Dispatchers.Main) {
        ...
        manager.location?.takeIf { it.horizontalAccuracy >= 0 }?.toGeoFix()?.takeIf { it.isFresh() }
    }
