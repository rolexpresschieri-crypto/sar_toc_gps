...
class GuidaPdf(FPDF):
    ...
    def footer(self) -> None:
        ...
        self.cell(96, 8, f"v. {VERSION}   pag. {self.page_no()}", align="R")
...
def build() -> Path:
    ...
    pdf.add_font("ArialIt", "I", str(FONT_REG))
    ...
    pdf.cell(0, 7, f"App Android  ·  versione {VERSION}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 7, "Dal log-in al modulo GPS (mappa, waypoint, tracce, misura)", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(14)
    ...
    pdf.multi_cell(
        0,
        6,
        "Questa guida spiega tutte le funzioni disponibili oggi sull’app TOC SAR, "
        "partendo dal log-in e dal log-out, fino al modulo GPS. "
        "Serve all’operatore sul campo: cosa fa ogni tasto, quando usarlo e cosa succede.",
        align="C",
    )
    pdf.ln(16)
    ...
    pdf.multi_cell(
        0,
        5.5,
        "Login = tracking verso il TOC. La posizione continua con schermo spento e telefono in tasca "
        "(notifica «Tracking operatore»). Il log-out è solo esplicito: tasto Log-out o force log-out da admin (LUPO).",
        align="C",
    )
    ...
    pdf.note(
        "Il tracking non richiede di tenere l’app aperta in mano. Dopo il log-in resta la notifica fissa "
        "«Tracking operatore»: posizione verso TOC e, se attivo, TRK anche in tasca."
    )
    ...
    pdf.bullet(
        ...
        "Apre le impostazioni Android dell’app (permessi, batteria, notifiche).",
    )
    pdf.bullet(
        "Log-in",
    ...
    pdf.p(
        ...
        "notifica tracking tolta, sparisci dalla mappa degli altri operatori. Per tornare visibili: di nuovo Log-in."
    )
    pdf.p(
    ...
    pdf.bullet(
        ...
        "La registrazione continua in tasca e a schermo spento (stesso servizio di tracking).",
    )
    pdf.bullet(
        "STOP TRK",
    ...
    pdf.p(
        ...
        "lo vedi, poi in mappa scegli tu cosa misurare (tap sul pin)."
    )
    pdf.note(
    ...
    pdf.bullet("Pallino colorato + codice", "Altro operatore online visibile (vedi cap. 6).")
    ...
    pdf.bullet("POSIZIONE ATTUALE", "Riquadro in basso a destra: coordinate, accuratezza, velocità, orario fix.")
    ...
    pdf.multi_cell(
        0,
        5.4,
        "Fine della guida per lo stato attuale dell’app (fino al GPS). "
        "Funzioni successive (oltre GPS) verranno aggiunte in questa stessa serie di documenti PDF.",
    )
